...
void showUsage()
{
  ...
  cerr << "1. vrg3d-server [-nogfx]  [-share <path>]  known-setup-name" << endl;
  cerr << "2. vrg3d-server [-nogfx]  [-share <path>]  device-filename [numclients] [port]" << endl;
  cerr << endl;
...
void shutdownServer()
{
   // This note was here, but may now be fixed:
  ...
  virtual void doUserInput(Array<VRG3D::EventRef> &events)
  {
    ...
    { if (events[i]->getName() == "kbd_ESC_down")
      { events.append(new Event("Shutdown"));
        ...
        // Wait for server threads to shutdown.
        shutdownServer();
...
public:
  ...
  virtual void waitForEventRequest()
  {
    ...
    { checkConnections();
      ...
      // Start unwinding if shutting down.
      gShutdownLock.lock();
  ...
  void mainloop()
  {
    ...
    // Unwind if waitForEventRequest exited on a shutdown.
    waitForEventRequest();
    gShutdownLock.lock();
...
int main(int argc, char **argv)
{
  ...
  // xxx - fix this - without creating 'dummy' the static method
  // VRG3D::findVRG3DDataFile can't find vrsetup files because
  // the static array _dataFilePaths is initialized in
  // the VRApp constructor.  Perhaps instead if the length of
  // _dataFilePaths is zero it should get initialized in
  // VRG3D::findVRG3DDataFile itself (?) .
  //
  if ( std::string( argv[i] ) == "-share" )
  {  i++;
     ...
     share = std::string( argv[i] );
     i++;
  }
  else
  {  vrg3d_share = getenv( (char *)"VRG3D_SHARE" );

     if ( vrg3d_share )
     {  share = std::string( vrg3d_share );
     }
  }

  ///cout << ":::::::::: share = " << share << endl;
  VRApp *dummy = new VRApp( share );
  ...
  { cout<< "Welcome to MSI lab"<<endl;
    numClients = 3;
    gDeviceFile = VRApp::findVRG3DDataFile("msi-devices.cfg");
  }
  else if (argStr == "mttablewall") {
    cout<< "Using multitouch table wall devices";
    numClients = 2;
    gDeviceFile = VRApp::findVRG3DDataFile("mttablewall-devices.cfg");
  }
  // here, no pre-defined setup, so customize based on arguments
  else
  { gDeviceFile = VRApp::findVRG3DDataFile( argStr );

    if ( gDeviceFile == "" )
    {  cerr << "The given device file (" << argStr << ") could not be found." << endl;
       exit(1);
    }
  ...
  gDeviceThread = GThread::create( "DeviceInputThread",
                                   device_input_thread_func,
                                   param );
  ...
  { // The main program thread will be used to run the graphics
    ...
    // server.
    gServerThread = GThread::create("ServerThread", server_thread_func, param);

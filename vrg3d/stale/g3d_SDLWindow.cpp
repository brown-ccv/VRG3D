...
void SDLWindow::setIcon(const GImage& image) {
    ...
    G3D::uint32 amask = 0xFF000000;
    G3D::uint32 bmask = 0x00FF0000;
    G3D::uint32 gmask = 0x0000FF00;
    G3D::uint32 rmask = 0x000000FF;
...
void SDLWindow::getRelativeMouseState(Vector2& p, G3D::uint8& mouseButtons) const {
    int x, y;
...
void SDLWindow::getRelativeMouseState(int& x, int& y, G3D::uint8& mouseButtons) const {
    mouseButtons = SDL_GetMouseState(&x, &y);
}


void SDLWindow::getRelativeMouseState(double& x, double& y, G3D::uint8& mouseButtons) const {
    int ix, iy;
...
void SDLWindow::myReallyMakeCurrent() const {
    reallyMakeCurrent();
}

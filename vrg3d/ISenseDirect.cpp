...
ISenseDirect::ISenseDirect( std::string   name,
                            Log          *log,
                            ConfigMapRef  map )
{
  ...
  for (int i = 0; i < ISD_MAX_STATIONS; i++)
  { std::string bEventsStr = map->get( name + "_ButtonEventsToGenerate" + intToString(i), "" );
  ...
  ISenseDirect( events, scale, d2r, p2t, fo, bevents );
}
...
ISenseDirect::pollForInput(Array<EventRef> &events)
{
  ...
  ISD_GetTrackingData( _handle, &trackerData );

  for (int s=0;s<_maxStations;s++) {
    ISD_STATION_DATA_TYPE *data = &trackerData.Station[s];
    ...
    if ( _stationInfo[s].AngleFormat == ISD_QUATERNION ) {
      ...
      Matrix3 rot = Matrix3(Quat(data->Quaternion[0], data->Quaternion[1],
      ...
      Matrix3 rotY = Matrix3::fromAxisAngle(Vector3(0,1,0), toRadians(data->Euler[1]));
      Matrix3 rotX = Matrix3::fromAxisAngle(Vector3(1,0,0), toRadians(data->Euler[0]));
      Matrix3 rotZ = Matrix3::fromAxisAngle(Vector3(0,0,1), toRadians(data->Euler[2]));

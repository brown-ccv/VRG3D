...
static void glutToSDLKeyEvent(GEvent& e, unsigned char c)
{
    ...
    ///e.key.keysym.mod = (G3D::GKeyMod)KMOD_NONE;
    e.key.keysym.mod = (G3D::GKeyMod::Value)KMOD_NONE;

    int mods = glutGetModifiers();

    if (mods & GLUT_ACTIVE_SHIFT) {
        ///e.key.keysym.mod = (G3D::GKeyMod)(e.key.keysym.mod | KMOD_LSHIFT);
        e.key.keysym.mod = (G3D::GKeyMod::Value)((int)e.key.keysym.mod | KMOD_LSHIFT);
    }

    if (mods & GLUT_ACTIVE_CTRL) {
        ///e.key.keysym.mod = (G3D::GKeyMod)(e.key.keysym.mod | KMOD_LCTRL);
        e.key.keysym.mod = (G3D::GKeyMod::Value)((int)e.key.keysym.mod | KMOD_LCTRL);
    }

    if (mods & GLUT_ACTIVE_ALT) {
        ///e.key.keysym.mod = (G3D::GKeyMod)(e.key.keysym.mod | KMOD_LALT);
        e.key.keysym.mod = (G3D::GKeyMod::Value)((int)e.key.keysym.mod | KMOD_LALT);
    }
...
void GlutWindow::g_keyboardspecial(int key, int x, int y) {
   ...
   glutToSDLKeyEvent(e, 'A');
   if (key == GLUT_KEY_LEFT) {
      e.key.keysym.sym = G3D::GKey::LEFT;
   }
   else if (key == GLUT_KEY_RIGHT) {
      e.key.keysym.sym = G3D::GKey::RIGHT;
   }
   else if (key == GLUT_KEY_UP) {
      e.key.keysym.sym = G3D::GKey::UP;
   }
   else if (key == GLUT_KEY_DOWN) {
      e.key.keysym.sym = G3D::GKey::DOWN;
   }
   currentGlutWindow->postEvent(e);
...
void GlutWindow::g_mousebtn(int b, int s, int x, int y) {
    ...
    case GLUT_LEFT_BUTTON:
        e.button.button = SDL_BUTTON_LEFT - 1;
...
GlutWindow::GlutWindow(const GWindow::Settings& s)
{
    ...
    settings.msaaSamples = glutGet(GLUT_WINDOW_NUM_SAMPLES);

...
void VideoOutput::initialize(const std::string& filename, const Settings& settings) {
    ...
    // TODO: ffmpeg 1.0 - did this move? is it needed? was it ever needed?
    ...
    int avRet = avcodec_open(m_avStream->codec, codec);
...
void VideoOutput::append(RenderDevice* rd, bool backbuffer) {
	...
	rd->screenshotPic(m_tempImg, false, false);
    rd->setReadBuffer(old);
	encodeFrame(const_cast<uint8*>(m_tempImg.byte()), ImageFormat::RGB8(), true);
}
...
void VideoOutput::append(const Texture::Ref& frame, bool invertY) {
    ...
    frame->getImage(m_tempImg, TextureFormat::RGB8());
    encodeFrame(const_cast<uint8*>(m_tempImg.byte()), ImageFormat::RGB8(), false);
}

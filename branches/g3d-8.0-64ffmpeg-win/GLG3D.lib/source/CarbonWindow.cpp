...
#ifdef __x86_64__
  #error Carbon not supported on 64-bit arches
#endif
#ifdef __LP64__
  #error Carbon doesn't work on 64-bit macs
#endif
#ifndef __CARBON__
  #error Carbon somehow not included
#endif
...
(const GWindow::Settings& s, 
 bool creatingShareWindow) : 
    ...
    if (m_settings.fullScreen) {
        ...
        // Jul 29, 2012: David: commenting out this call since it fails on 
        // OS X 10.7 and later.  This renders fullscreen completely broken
        // with CarbonWindow.
...
#ifdef __clang__
    Rect rWin = {static_cast<short>(m_settings.y), static_cast<short>(m_settings.x), static_cast<short>(m_settings.height+m_settings.y), static_cast<short>(m_settings.width+m_settings.x)};
#else

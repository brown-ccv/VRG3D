...
void
EventMgr::queueEvent(VRG3D::EventRef event)
{
...
void
EventMgr::queueEventWithoutFilter(VRG3D::EventRef event)
{
  // queue up the event
  ...
  if (_eventAliases.containsKey(event->getName())) {
    ...
    for (int i=0;i<aliases.size();i++) {
      VRG3D::EventRef newevent = createCopyOfEvent(event);
...
void
EventMgr::processEvent(VRG3D::EventRef event)
{
...
void
EventMgr::processEventDeviceLevel(VRG3D::EventRef event)
{
  if ((_printAsProcessed) || (_fullDebug))
...
void
EventMgr::queueTimerEvent(VRG3D::EventRef event, double queueTime)
{
...
EventMgr::getCurrentHeadFrame()
{
  ...
  VRG3D::EventRef lastHeadMovement = getMostRecentEvent(ConfigVal("HeadTracker", "Head_Tracker", false));
